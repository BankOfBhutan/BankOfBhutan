...
const generateCustomToken = async (atsDate) => {
    ...
    const lastATS = await ATS.findOne({
        token: { $regex: `^OATS` } // Match tokens starting with ODT + formattedDate (YYYYMMDD)
    })
    ...
    if (lastATS) {
        ...
        const lastATSDate = new Date(lastATS.atsDate)
            .toISOString()
            .split('T')[0]
            .replace(/-/g, '');

        if (lastATSDate === formattedDate) {
            // If the dates match, increment the unique number
            const lastUniqueNumber = parseInt(lastATS.token.slice(-2), 10); // Get the last 2 digits of the token
            uniqueNumber = lastUniqueNumber + 1;
        }
    }

    const formattedUniqueNumber = uniqueNumber.toString().padStart(2, '0');
    const token = `OATS${formattedUniqueNumber}`;
...
exports.submitATS = async (req, res) => {
    ...
    try {
        ...
        if (err.code === 11000) {
            ...
            } else if (conflictingField === 'atsTime') {
...
exports.checkConflict = async (req,res)=>{
    const { email, atsDate, atsTime } = req.body;
    try {
        const emailConflict = await ATS.findOne({ email, atsDate });
        const timeConflict = await ATS.findOne({ atsDate, atsTime });
