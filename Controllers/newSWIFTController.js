...
const generateCustomToken = async (swiftDate) => {
    ...
    const lastswift = await SWIFT.findOne({
    ...
    if (lastswift) {
        ...
        const lastswiftDate = new Date(lastswift.swiftDate)
            .toISOString()
            .split('T')[0]
            .replace(/-/g, '');
...
exports.submitswift = async (req, res) => {
    const { service, Name, email, accountNumber, contact, swiftDate, swiftTime,otp } = req.body;
    
    if (!otp || otp !== otpStore[email]?.otp) {
        return res.status(400).json({ message: 'Invalid or missing OTP' });
      }
    try {
        const token = await generateCustomToken(swiftDate); // Add await here
        ...
        const mailOptions = {
            ...
            subject: 'swift Token Booking Confirmation',
            html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                <h2 style="color: #333; text-align: center;">swift Token Booking Confirmation</h2>
                ...
                <div style="background-color: #f9f9f9; padding: 10px; border-radius: 5px; margin-top: 10px;">
                    <ul style="list-style-type: none; padding: 0;">
                        ...
                        <li><strong>Date:</strong> ${swiftDate}</li>
                        <li><strong>Time:</strong> ${swiftTime}</li>
                    </ul>
                </div>
        ...
        res.status(200).json({ message: 'swift Token successfully booked!', token });
        ...
        if (err.code === 11000) {
            ...
            } else if (conflictingField === 'swiftTime') {
            ...
            res.status(500).json({ error: 'An error occurred while submitting the swift' });
        }
...
exports.checkConflict = async (req,res)=>{
    ...
    try {
        const emailConflict = await SWIFT.findOne({ email, swiftDate });
        const timeConflict = await SWIFT.findOne({ swiftDate, swiftTime });
